...
func TestTracingStatusBasicOperation(t *testing.T) {
	...
	ts := MakeTracingStatus("test-node-2", ch)
	...
	expTs := `{"nodeName":"test-node-2","data":{"fingerprintExpected":"random-value-chk","fingerprintComputed":"random-value-sign","pods":[{"Namespace":"ns-1","Name":"pod-1"},{"Namespace":"ns-2","Name":"pod-2"},{"Namespace":"ns-3","Name":"pod-3"}],"nodeName":"test-node-2"},"lastModified":"2006-01-02T15:04:05Z"}`
	if string(data) != expTs {
		t.Errorf("unexpected data from chan:\n%s\nexpected:\n%s", string(data), expTs)
	}

	retRepr := ret.Data.Repr()
	expRepr := `> processing node "test-node-2"
> processing 3 pods
+ ns-1/pod-1
+ ns-2/pod-2
+ ns-3/pod-3
...
`
	...
	expOrigRepr := `> processing node "test-node-2"
> processing 2 pods
+ ns-1/pod-1
+ ns-2/pod-2
= random-value-sign
